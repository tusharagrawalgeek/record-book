...
function Export() {
  ...
  function exportItem(item, quantity) {
    ...
    var p = new Promise(async (resolve, reject) => {
      await axios
        .post(url + "/exportitem", data)
        .then(async (res) => {
          console.log(res);
          if (item.quantity - quantity === 0) {
            axios
              .delete(url + "/deleteitem/" + item._id)
              .then((res) => {
                console.log(res);
                if (res === 0) {
                  reject(0);
            ...
            await axios
              .put(url + "/update/" + item._id, updateData)
              .then((res) => {
                console.log(res);
                resolve(1);
              })
  ...
  return (
    <>
      ...
      >
        <div>
          <h2 style={{ marginTop: "0", color: color.contrast }}>
            EXPORTED ITEMS{" "}
          </h2>
        </div>
        <div
        ...
        >
          ...
          {state.showExportForm1 && <>{showPreForm()}</>}
          {state.showExportForm2 && <>{showFullForm()}</>}
        </div>
      </div>
    </>
  );
          ...
          <tr>
            ...
            <td colSpan="3" className="td-add-btn">
              <button
                ...
                name="addItem"
                disabled={state.exportItemCount > state.selectValues.length}
              >
                +
              </button>
            </td>
    ...
    return (
      <>
        ...
        >
          ...
          <tr>
            <td className="tditem">EXPORT DATE</td>
          ...
          <tr>
            <td className="tditem">EXPORT TO</td>
            <td className="tditem">:</td>
            <td className="tditem">
              <input
                className="inputitem"
          ...
          <tr>
            <td className="tditem">PURPOSE</td>
            <td className="tditem">:</td>
            <td className="tditem">
              <input
                className="inputitem"
